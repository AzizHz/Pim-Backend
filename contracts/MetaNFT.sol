...
contract MetaNFT is ERC721Enumerable , Ownable {
   ...
   uint256 public cost = 1 * 10**18; // 1 HAR
  //HARToken public harToken;
   ...
   function payToMint(
        ...
        // require(
        //      harToken.transferFrom(msg.sender, address(this), cost),
        //     "Transfer failed. You may not have enough HAR tokens to perform this action."
        // );
        require(existingURIs[metadataURI] == 0, "This NFT is already minted!");

        uint256 royality = (cost * royalityFee) / 100;
        payTo(artist, royality);
        payTo(owner(), (cost - royality));
