...
router.get('/players', (req, res) => {
    ...
    axios.get(url2, config)
        .then(response => {
            ...
            // Create an array of JSON objects from the rowsets data
            const jsonObjects = [];

            // Create a JSON object for each row in the rowset
            for (const row of rowsets) {
                const jsonObject = { PERSON_ID: row[0], PLAYER_LAST_NAME: row[1], PLAYER_FIRST_NAME: row[2], PLAYER_SLUG: row[3], TEAM_ID: row[4], TEAM_SLUG: row[5], IS_DEFUNCT: row[6], TEAM_CITY: row[7], TEAM_NAME: row[8], TEAM_ABBREVIATION: row[9], JERSEY_NUMBER: row[10], POSITION: row[11], HEIGHT: row[12], WEIGHT: row[13], COLLEGE: row[14], COUNTRY: row[15], DRAFT_YEAR: row[16], DRAFT_ROUND: row[17], DRAFT_NUMBER: row[18], ROSTER_STATUS: row[19], PTS: row[20], REB: row[21], AST: row[22], STATS_TIMEFRAME: row[23], FROM_YEAR: row[24], TO_YEAR: row[25] };
